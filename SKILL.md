...
description: |
  在任何中国地图类报告/可视化中，用官方标准数据绘制合规的九段线（南海断续线）+ 南海诸岛方形附图。
  支持三种使用方式：(A) 一站式生成分级图 (B) 给已有 matplotlib 主图补附图 (C) 给已有图片文件(PNG/JPG)自动采样配色并合成同色系附图 (D) 叠加自有 SHP/GeoJSON 专题图层组合出图。
  禁止手绘近似九段线坐标——本 skill 用 frykit 官方标准坐标。也解决"一张图里出现两幅中国地图"的附图错误与中文乱码。
agent_created: true
---
